# react-native-share-edoc2 [![npm version](https://badge.fury.io/js/react-native-share-edoc2.svg)](http://badge.fury.io/js/react-native-share-edoc2)
...
1. `npm install react-native-share-edoc2 --save`
2. `react-native link`
...
3. Go to `node_modules` ➜ `react-native-share-edoc2` ➜ `ios` and add `RNShare.xcodeproj`
...
3. Append the following lines to `android/settings.gradle`:
  	```
  	include ':react-native-share-edoc2'
  	project(':react-native-share-edoc2').projectDir = new File(rootProject.projectDir, 	'../node_modules/react-native-share-edoc2/android')
  	```
4. Insert the following lines inside the dependencies block in
   `android/app/build.gradle`:

    ```
      compile project(':react-native-share-edoc2')
    ```
5. Follow this
...
6. Edit your `MainApplication` class to implement `ShareApplication`
  ...
  - For example: Replace the `com.example.yourappidhere` below with the
    ...
    properly](https://github.com/EstebanFuentealba/react-native-share-edoc2/issues/200#issuecomment-361938532).
...
1. `npm install react-native-share-edoc2 --save`
2. In Visual Studio add the `RNShare.sln` in `node_modules/react-native-share-edoc2/windows/RNShare.sln` folder to their solution, reference from their app.
...
import Share, {ShareSheet, Button} from 'react-native-share-edoc2';
